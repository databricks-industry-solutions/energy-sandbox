...
- **Pipeline Advisor (Genie)** — Natural-language chat over UC pipeline tables (compressor vibration, RUL, work orders, anomalies); falls back to the local PipelineGuardian agent on Genie error
- **Floating Genie panel** — Always-on natural-language sidebar scoped to the demo's gold schema, available across every tab
...
| **Pipeline Advisor** | Genie-powered chat over UC pipeline tables (compressor vibration, RUL, work orders) with natural-language SQL and conversation memory; falls back to the local PipelineGuardian agent on Genie error |
...
A portable `databricks.yml` is included. Set your workspace + warehouse variables and:
...
See [`DEPLOY.md`](DEPLOY.md) for the full walkthrough, including required env vars (`GENIE_SPACE_ID`, `DATABRICKS_HOST`) and how to register the Genie space.

### Deploy manually (without DABs)
