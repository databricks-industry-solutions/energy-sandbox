"""Genie Conversation API proxy for LAS Viewer.

Scoped to the demo's Genie Space (env: GENIE_SPACE_ID) covering the
LAS Viewer gold tables in Unity Catalog (wells, depth_logs, curve_quality,
anomalies, processing_runs, formation_tops, etc.).

Prereq: a Genie Space scoped to a UC schema (e.g. `oil_pump_monitor_catalog.las_viewer`)
containing the gold tables above. If GENIE_SPACE_ID is unset, the route returns
a configuration error so the UI can prompt the operator to set up the space.
...
async def space_info():
    return {
        ...
        "name": "LAS Viewer — Fleet & Inspections Genie",
