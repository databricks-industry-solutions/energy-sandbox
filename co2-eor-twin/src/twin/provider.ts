import { TwinState, Well, Facility, Pipeline, FlarePoint, MonitoringPoint } from './types';
...
function driftWell(w: Well, base: Well): void {
  if (w.type === 'producer') {
...
function driftFacility(f: Facility, base: Facility): void {
...
function driftPipeline(p: Pipeline, base: Pipeline): void {
...
function driftFlare(f: FlarePoint, base: FlarePoint): void {
  if (f.status === 'active') {
    f.currentRate = rd(clamp(f.currentRate + jitter(15), 0, base.maxRate * 0.15), 0);
  }
}

function driftMonitor(m: MonitoringPoint, base: MonitoringPoint): void {
...
function recalcKPIs(s: TwinState): void {
...
function applyDrift(state: TwinState, baseline: TwinState): void {
...
export class InMemoryTwinDataProvider implements TwinDataProvider {
  private state: TwinState;
  private baseline: TwinState;
  constructor() {
    this.baseline = createMockState();
    this.state = createMockState();
  }
  async loadState(): Promise<TwinState> {
...
export class DatabricksTwinDataProvider implements TwinDataProvider {
  async loadState(): Promise<TwinState> {
