...
import { Well, Facility, FleetAsset } from '../twin/types';

const router = Router();
const provider = new InMemoryTwinDataProvider();

function wellColor(type: Well['type']): string {
  switch (type) {
...
function facilityColor(utilization: number): string {
  if (utilization > 0.85) return '#ef4444';   // red
  if (utilization > 0.70) return '#eab308';   // yellow
  return '#22c55e';                           // green
}

function pipelineColor(product: string): string {
  switch (product) {
...
function fleetIcon(type: FleetAsset['type']): string {
  switch (type) {
