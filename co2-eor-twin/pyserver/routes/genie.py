"""Genie Conversation API proxy for the CO2-EOR Digital Twin.

Scoped to a Genie Space covering the CO2-EOR gold/silver tables (env: GENIE_SPACE_ID).
...
def _ask_sync(question: str, conversation_id: str | None, status_cb=None):
    if not GENIE_SPACE_ID:
        return {"error": "GENIE_SPACE_ID not configured. Create a Genie Space scoped to the CO2-EOR gold tables and set the env var."}
...
async def space_info():
    return {
        ...
        "name": "CO2-EOR Digital Twin Genie",
