"""Genie Conversation API proxy for Reservoir Simulator.

Scoped to the demo's Genie Space (env: GENIE_SPACE_ID) covering the Norne
field gold tables in Unity Catalog.
...
def _ask_sync(question: str, conversation_id: str | None, status_cb=None):
    if not GENIE_SPACE_ID:
        return {"error": "GENIE_SPACE_ID not configured. Create a Genie Space scoped to the demo's gold tables and set the env var."}
...
async def space_info():
    return {
        ...
        "name": "Reservoir Simulator — Norne Field Genie",
