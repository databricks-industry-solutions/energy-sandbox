...
export interface WellEconomics {
  ...
  oilRevenue: number;       // $/d
  gasRevenue: number;       // $/d
  co2Cost: number;          // $/d (allocated)
  loe: number;              // $/d lease operating expense
  transportCost: number;    // $/d
  netbackPerBoe: number;    // $/boe
  co2PerBoe: number;        // mcf CO2/boe
}
...
export interface CO2Contract {
  ...
  volume: number;           // mcf/d
  price: number;            // $/mcf
  takeOrPay: number;        // mcf/d minimum
...
export interface CarbonCredit {
  vintage: number;
  registry: string;
  pricePerTon: number;      // $/tCO2
  volumeAvailable: number;  // tCO2
  projectId: string;
  methodology: string;
}
...
export interface FieldEconomicsSummary {
  ...
  fieldNetback: number;         // $/boe
  incrementalNetback: number;   // $/boe for EOR increment
  breakeven: number;            // WTI $/bbl
  carbonCreditRevenue: number;  // $/d
  totalBoe: number;
  wellCount: number;
}
...
export function getWellEconomics(state: TwinState): WellEconomics[] {
...
export function getCO2Contracts(): CO2Contract[] {
  return [
    {
...
export function getFieldEconomicsSummary(state: TwinState): FieldEconomicsSummary {
